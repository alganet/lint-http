...
impl ProtocolRule for WebsocketFrameMasking {
    fn findings(
        ...
        let finding = || -> Option<Violation> {
            ...
            // declines. A message-level record carries no frame header for the bit
            // to come from, and every event written before the field existed reads
            ...
            Some(
                ctx.by_direction(*direction)
                    .report_with(def, format!("A WebSocket frame where {defect}")),
            )
        };
