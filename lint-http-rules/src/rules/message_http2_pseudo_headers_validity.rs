...
mod tests {
    ...
    /// and is `server_status_code_valid_range`'s finding. Both statuses here
    ...
    fn scope_is_both() {
        assert_eq!(
            MessageHttp2PseudoHeadersValidity.scope(),
            crate::rules::RuleScope::Both
        );
    }
    ...
    fn id_returns_correct_value() {
        assert_eq!(
            MessageHttp2PseudoHeadersValidity.id(),
            "message_http2_pseudo_headers_validity"
        );
    }
    ...
    fn validate_rules_with_valid_config() -> anyhow::Result<()> {
        ...
        crate::test_helpers::enable_rule(&mut cfg, "message_http2_pseudo_headers_validity");
