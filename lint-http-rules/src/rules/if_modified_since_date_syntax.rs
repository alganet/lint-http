...
/// field's. `If-Modified-Since = HTTP-date` and a request is a sender, so the
/// same pair `Last-Modified` reports answers here — the obsolete formats a
...
"#
    ...
    fn specifications(&self) -> &'static [crate::rules::SpecRef] {
        &[RFC_9110_13_1_3, RFC_9110_5_6_7]
    }
                ...
                for hv in tx.request.headers.get_all("if-modified-since").iter() {
                    ...
                    if crate::helpers::headers::trim_ows(s).is_empty() {
                        return Some(self.violation(
                            ctx.severity,
                            "If-Modified-Since header is empty or contains only whitespace".into(),
                        ));
                    }
