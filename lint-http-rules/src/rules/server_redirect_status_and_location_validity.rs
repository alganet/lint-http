...
fn location_has_a_referent(status: u16) -> bool {
    ...
    // there. Whether a 201 *ought* to carry one is `semantic_post_creates_resource`'s
    ...
    // `server_status_code_valid_range`'s finding, not this one's. `resp.status` can
...
impl Rule for ServerRedirectStatusAndLocationValidity {
    ...
    fn check_transaction(
        ...
        // the two rules that know it (`server_3xx_vs_request_method`,
        // `semantic_post_creates_resource`); nothing here varies with it.
        ...
        // `server_location_header_uri_valid`'s questions, and none of them changes
...
mod tests {
    ...
    /// `Location` is a usable `URI-reference` is `server_location_header_uri_valid`'s
    ...
    /// about the field's syntax, which `server_location_header_uri_valid` owns.
    #[test]
