...
///
/// This rule reads the *trailer section*; `trailer_header_valid` reads the
...
/// answered for writing one by `trailer_header_valid`.
///
...
static REGISTRATION: &dyn crate::rules::Rule = &TrailerFieldsValid;
...
mod tests {
    ...
    #[case("connection")]
    #[case("keep-alive")]
    // §7.6.1's fifth connection-specific name, the one the table omitted while
    // `CONNECTION_SPECIFIC_FIELDS` held it (RULECITES P46).
    ...
    fn validate_rules_with_valid_config() -> anyhow::Result<()> {
        ...
        crate::test_helpers::enable_rule(&mut cfg, "trailer_fields_valid");
