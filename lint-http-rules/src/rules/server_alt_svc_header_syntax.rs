...
mod tests {
    ...
    fn config() -> crate::config::Config {
        crate::test_helpers::make_test_config_with_severity("server_alt_svc_header_syntax", "warn")
    }

    fn check(tx: &crate::http_transaction::HttpTransaction) -> Option<Violation> {
        ServerAltSvcHeaderSyntax.check_transaction(
            tx,
    ...
    fn scope_is_server() {
        assert_eq!(
            ServerAltSvcHeaderSyntax.scope(),
            crate::rules::RuleScope::Server
        );
    }
}
