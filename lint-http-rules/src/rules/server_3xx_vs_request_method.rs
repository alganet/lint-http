...
impl Rule for Server3xxVsRequestMethod {
    ...
    fn check_transaction(
        ...
        // `server_location_header_uri_valid`'s, and a 301 or a 302 carrying no
        // `Location` at all is `server_response_location_on_redirect`'s, against the
        ...
        // *that* is `client_request_method_token_valid`'s to report.
...
mod tests {
    ...
    /// note is not about. `client_request_method_token_valid` reports the lowercase
    ...
    /// The missing field is a finding, but it is `server_response_location_on_redirect`'s.
    #[rstest]
    #[case(301)]
    #[case(302)]
