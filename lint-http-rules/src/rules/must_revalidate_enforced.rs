...
mod tests {
    ...
    /// A `412` carrying the directive and an `ETag` is not an entry, because
    /// § 3's last term refuses it: no freshness, no `public` or `private`, and
    /// a status no cache may keep heuristically. The same response at 200 is.
    #[rstest::rstest]
    #[case(412, false)]
    #[case(200, true)]
    fn a_response_no_cache_could_keep_is_no_entry(
        #[case] status: u16,
        #[case] expect_finding: bool,
    ) {
        let rule = MustRevalidateEnforced;
