...
impl Rule for ServerNoBodyFor1xx204304 {
    ...
    fn check_transaction(
        ...
        // both were run rather than read. `message_response_body_length_accuracy`
        ...
        // three statuses over. `server_http3_status_code_validity` had all three of
        ...
        //
        // `message_trailer_fields_validity` judges *which* fields a trailer section
        // may hold. Whether the response was allowed a trailer section at all is
        // this rule's question. `server_http3_status_code_validity` asked it for
        ...
        // sentence does not ask. `message_content_length` owns the field's syntax.
        //
        ...
        // `message_http3_no_connection_header` reports it for HTTP/3 -- reporting it
        // here as well would be two findings for one field. HTTP/2 has no such rule
        // yet, so an HTTP/2 1xx or 204 carrying the field goes unreported; that gap
        // is the field's, not this status set's, and `description()` names it.
    ...
    fn specifications(&self) -> &'static [crate::rules::SpecRef] {
        &[
            ...
            crate::rules::SpecRef {
                ...
                note: "HTTP/3 — transfer codings are not defined and the field must not be used; message_http3_no_connection_header reports it. The same section repeats the trailers half for interim responses",
            },
        ]
    }
...
mod tests {
    ...
    fn run(tx: &crate::http_transaction::HttpTransaction) -> Option<Violation> {
        ServerNoBodyFor1xx204304.check_transaction(
            tx,
    ...
    fn scope_is_server() {
        assert_eq!(
            ServerNoBodyFor1xx204304.scope(),
            crate::rules::RuleScope::Server
        );
    }
    ...
    fn validate_rules_with_valid_config() -> anyhow::Result<()> {
        ...
        crate::test_helpers::enable_rule(&mut cfg, "server_no_body_for_1xx_204_304");
