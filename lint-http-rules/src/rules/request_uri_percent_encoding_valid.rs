...
impl RuleMeta for RequestUriPercentEncodingValid {
...
static REGISTRATION: &dyn crate::rules::Rule = &RequestUriPercentEncodingValid;
...
mod tests {
    ...
    /// method-specific forms, which are `request_target_form_valid`'s
    ...
    /// and not only in a capture file read back through `lint-captures`. The characters
