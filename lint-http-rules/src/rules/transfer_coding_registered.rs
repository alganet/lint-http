...
/// general licence for an `x-` prefix. `chunked` is the sixth, and says it about
/// itself at the end of § 7.1.
const PARAMETERLESS_CODINGS: [&str; 6] = [
    "compress",
    "x-compress",
    "deflate",
    "gzip",
    ...
    fn findings(
        ...
        let mut out = Vec::new();
        {
            ...
            let check_value = |hdr_name: &str,
             ...
             -> Vec<Violation> {
                // One finding per member. `#transfer-coding` names one coding
                // per position -- each a transformation applied to this
                // message's body, or one the client will accept -- so a value
                // naming two the recipient cannot act on is two names to
                ...
                if !crate::helpers::list::quoting_is_balanced(val) {
                    // The one branch that still answers for the whole value,
                    // and it has to: with the quoting unclosed the splitter
                    // cannot say where the members are, so there is no second
                    // member to report on.
                ...
                for part in crate::helpers::list::split_commas_respecting_quotes(val) {
                    ...
                    {
                        found.push(ctx.by(party).report(&TE_CHUNKED_FORBIDDEN));
                        continue;
                    }
...
static REGISTRATION: &dyn crate::rules::Rule = &TransferCodingRegistered;
...
mod tests {
    ...
    /// **Every coding the response named is answered.** `#transfer-coding`
    /// names one coding per position, each a transformation applied to this
    /// message's body, so a value naming two the recipient cannot act on is two
    /// names to correct.
    #[test]
    fn every_defective_coding_is_reported() {
        ...
        let tx = crate::test_helpers::make_test_transaction_with_response(
            200,
            &[("transfer-encoding", "g@zip, bogus, chunked")],
        );
        ...
        assert!(ids.contains(&"transfer_coding_unregistered"), "{ids:?}");
    }

    fn cfg_allowing(codings: &[&str]) -> crate::config::Config {
        let mut cfg = crate::config::Config::default();
        cfg.rules.insert(
            "transfer_coding_registered".into(),
            toml::Value::Table({
    ...
    // § 7.1: the chunked coding defines no parameters.
    #[case(Some("chunked; param=1"), true)]
    #[case(None, false)]
    ...
    fn invalid_token_in_transfer_encoding_is_reported() -> anyhow::Result<()> {
        ...
        assert!(v.is_some());
        let v = v.unwrap();
        assert_eq!(v.rule, "transfer_coding_registered");
        assert!(
    ...
    /// The codings that define no parameters -- § 7.2's five compression
    /// codings and § 7.1's `chunked` -- and the presence of one is to be treated
    /// as an error. Everything from the first `;` used to be discarded unread.
    #[rstest]
    ...
    // § 7.1's sentence, which an earlier pass here read as absent.
    #[case("transfer-encoding", "chunked;ext=1")]
    #[case("transfer-encoding", "gzip, chunked;ext=\"a,b\"")]
    fn a_parameter_on_a_coding_that_defines_none_is_reported(
        #[case] field: &str,
        #[case] value: &str,
    ) {
        let rule = TransferCodingRegistered;
    ...
    /// The exemptions: the TE rank is a `weight` and not a transfer-parameter,
    /// and a configured coding's parameters belong to whatever defines it.
    #[rstest]
    ...
    #[rstest]
    #[case("x-custom;ext=\"a,b\"")]
    // `x-custom` rather than a coding § 7.1 or § 7.2 declares parameterless:
    // this case is about the splitter, not about the parameter. It is in
    // `allowed` below by way of the config the test builds.
    #[case("x-custom;ext=\"a,b\", deflate")]
    #[case("x-custom;ext=\"a\\\",b\"")]
    fn a_comma_inside_a_quoted_parameter_value_is_not_a_separator(#[case] value: &str) {
        let rule = TransferCodingRegistered;
        // Its own `allowed`, holding a coding that § 7.1 and § 7.2 say nothing
        // about -- every coding in `make_cfg` is one of the six that may carry
        // no parameters, which would report these for the parameter rather than
        // let them test the splitter.
        let cfg = cfg_allowing(&["x-custom", "deflate"]);
