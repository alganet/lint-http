...
impl RuleMeta for WebsocketFrameRsvBits {
...
impl ProtocolRule for WebsocketFrameRsvBits {
    fn findings(
        ...
        let finding = || -> Option<Violation> {
            ...
            // Zero is what the production generates without a negotiation, and
            // what conforming traffic carries — so this branch ends the rule for
            // most frames. The relay records the bits as the wire spelled them,
            // so a set bit reaches the findings below live as well as through
            // `lint-captures` over a capture.
            //
