...
pub struct MessageUpgradeHeaderSyntaxValid;
...
/// `message_via_header_syntax_valid` reads `[ protocol-name "/" ] protocol-version`
...
impl MessageUpgradeHeaderSyntaxValid {
...
impl Rule for MessageUpgradeHeaderSyntaxValid {
    fn id(&self) -> &'static str {
        "message_upgrade_header_syntax_valid"
    }
    ...
    fn check_transaction(
        ...
        // `message_no_connection_specific_fields` makes it, and says in as many
        ...
        // asked by nobody. `message_connection_header_tokens_valid` reads its own
...
static REGISTRATION: &dyn crate::rules::Rule = &MessageUpgradeHeaderSyntaxValid;
...
mod tests {
    ...
    fn upgrade(section: Section, lines: &[&[u8]]) -> Option<Violation> {
        let rule = MessageUpgradeHeaderSyntaxValid;
    ...
    /// `message_no_connection_specific_fields`'s finding, and that rule reads no
    ...
    fn published_examples_are_judged_by_this_rule_and_by_the_one_that_owns_the_option() {
        ...
        let rule = MessageUpgradeHeaderSyntaxValid;
        let neighbour = crate::rules::message_connection_upgrade::MessageConnectionUpgrade;
    ...
    fn scope_is_both() {
        ...
        assert_eq!(
            MessageUpgradeHeaderSyntaxValid.scope(),
            crate::rules::RuleScope::Both
        );
    }
    ...
    fn validate_rules_with_valid_config() -> anyhow::Result<()> {
        ...
        crate::test_helpers::enable_rule(&mut cfg, "message_upgrade_header_syntax_valid");
