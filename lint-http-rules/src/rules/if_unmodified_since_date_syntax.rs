...
impl RuleMeta for IfUnmodifiedSinceDateSyntax {
    ...
    fn findings(
        ...
        let finding = || -> Option<Violation> {
            ...
            for hv in tx.request.headers.get_all("if-unmodified-since").iter() {
                ...
                // The trim is `OWS` and it is this side's, for the reason written at
                // the sibling `if_modified_since_date_syntax`: the validator
                // measures the whole string, and what puts a leading or trailing
                // `OWS` outside *this* value is the `field-line` production rather
...
static REGISTRATION: &dyn crate::rules::Rule = &IfUnmodifiedSinceDateSyntax;
...
mod tests {
    ...
    fn if_unmodified_since_cases(
        ...
        let rule = IfUnmodifiedSinceDateSyntax;
    ...
    fn validate_rules_with_valid_config() -> anyhow::Result<()> {
        ...
        crate::test_helpers::enable_rule(&mut cfg, "if_unmodified_since_date_syntax");
    ...
    fn id_and_scope_are_expected() {
        let r = IfUnmodifiedSinceDateSyntax;
        assert_eq!(r.id(), "if_unmodified_since_date_syntax");
