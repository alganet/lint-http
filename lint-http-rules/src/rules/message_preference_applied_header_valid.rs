...
static REGISTRATION: &dyn crate::rules::Rule = &MessagePreferenceAppliedHeaderValid;
...
mod tests {
    ...
    fn prefer_syntax_belongs_to_the_neighbouring_rule() {
        let neighbour = crate::rules::REGISTERED_RULES
            .iter()
            .find(|r| r.id() == "message_prefer_header_valid")
            .expect("message_prefer_header_valid is registered");
