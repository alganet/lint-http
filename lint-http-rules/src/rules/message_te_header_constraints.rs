...
impl MessageTeHeaderConstraints {
    ...
    fn check_members(&self, value: &str, config: &crate::rules::RuleConfig) -> Option<Violation> {
        ...
        for member in split_commas_respecting_quotes(value) {
            ...
            // and `message_transfer_coding_iana_registered` already measures TE's
    ...
    fn check_parameter(
        ...
        // The `"="` is not optional in either production, so a parameter written
        // without one is a `token` sitting where neither generates it.
        let Some((raw_name, raw_value)) = parameter.split_once('=') else {
            return violation(format!(
                "TE parameter '{}' in member '{}' has no value; a transfer-parameter is a name, an '=', and a token or quoted-string",
        ...
        // `BWS` is where the grammar admits whitespace for historical reasons only.
        // Removing it is the recipient's MUST and is what the two trims above do;
        // *generating* it is a sender's MUST NOT, and this rule measures the sender.
        //
        ...
        if name.len() != raw_name.len() || value.len() != raw_value.len() {
            return violation(format!(
                "TE parameter '{}' in member '{}' writes whitespace around its '='; the grammar admits it there as BWS, which a sender must not generate",
                parameter.escape_debug(),
                member.escape_debug()
            ));
        }
...
impl Rule for MessageTeHeaderConstraints {
    ...
    fn examples(&self) -> &'static [crate::rules::Example] {
        ...
        &[
            ...
            // `message_transfer_coding_iana_registered`'s to judge, so the
...
mod tests {
    ...
    fn the_trailers_keyword_takes_neither_parameter_nor_weight(#[case] value: &[u8]) {
        let v = te(value).expect("violation");
        assert!(v.message.contains("trailers"), "{}", v.message);
    }
    ...
    /// `message_transfer_coding_iana_registered` is what measures a coding name, for
    ...
    /// `BWS` is whitespace the grammar admits for historical reasons; a recipient
    /// MUST remove it and a sender MUST NOT write it.
    #[rstest]
    #[case(b"deflate;q =0.5")]
    #[case(b"deflate;q= 0.5")]
    #[case(b"deflate;ext = x")]
    fn whitespace_around_the_equals_is_bws_a_sender_must_not_generate(#[case] value: &[u8]) {
        let v = te(value).expect("violation");
    ...
    fn published_te_values_are_accepted_by_the_rule_that_owns_coding_names() {
        ...
        let owner = super::super::message_transfer_coding_iana_registered::MessageTransferCodingIanaRegistered;
        ...
        for ex in MessageTeHeaderConstraints.examples() {
