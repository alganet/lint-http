...
/// production rather than to the field.
///
/// `t-codings = "trailers" / ( transfer-coding [ weight ] )` and
/// `transfer-parameter = token BWS "=" BWS ( token / quoted-string )`, so a
/// member of this field is a list element, a `token`, a `quoted-string`, a
/// `parameter` and a `qvalue` assembled — and each of those has a subject that
/// already holds the defect. **A `transfer-parameter` is a `parameter` with
/// `BWS` printed around its `=`**: the two productions agree on the name, on
/// the `=` being mandatory and on `( token / quoted-string )`, and differ only
/// in whitespace, which is why the whitespace is the one part answering to a
/// subject of its own here.
///
/// What is left in the rule's own words is what the *field* says: `trailers`
/// taking neither parameter nor weight, the connection option a sender of `TE`
/// must send beside it, and a `TE` written in a response. Those are conversions
/// of their own.
...
impl TeHeaderValid {
    ...
    fn check_members(&self, value: &str, ctx: &crate::rules::RuleContext<'_>) -> Vec<Violation> {
        ...
        'member: for member in list_members_as_written(value) {
            if member.is_empty() {
                ...
                // nothing to the list -- which is one sentence for every list-valued
                // field, and the id every one of them reports under.
    ...
    fn check_connection_option(
        ...
        Some(ctx.by_client().report(&TE_CONNECTION_OPTION_MISSING))
    }
    ...
    fn check_parameter(
        ...
        // `=` names nothing -- the token floor, and the id that names it.
        // `find_invalid_token_char` answers `None` for the empty string — there is no
        // offending character to point at — so without this the member passed.
        if name.is_empty() {
        ...
        // opening with a DQUOTE is being written as the second. Which way it fails is
        // the production's own question, answered by the reader that owns it: the
        // typed one, so the id is the defect rather than a rendered sentence.
        if value.starts_with('"') {
            if let Err(defect) = check_quoted_string(value) {
                return Some(ctx.by_client().report_with(
...
"#
    ...
    /// **Thirteen findings about a request's `TE` and one about a response
    /// that carries the field at all.** `TE` states what the client will accept
    /// in a transfer coding, so every reading of its members, its parameters and
