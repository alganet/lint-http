...
impl MessageConnectionUpgrade {
    ...
    /// `message_te_header_constraints` in exactly this shape — and once more one
    /// protocol wide, since RFC 6455 § 4.1 restates it for a WebSocket handshake and
    /// `client_sec_websocket_headers_consistency` measures that. The special cases
    ...
    /// one. What that value may hold is not measured anywhere in this catalogue. The
    ...
    fn defect(headers: &hyper::HeaderMap, version: &str, direction: &str) -> Option<String> {
        ...
        // `message_connection_header_tokens_valid`'s question.
