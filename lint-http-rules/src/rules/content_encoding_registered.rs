...
impl Rule for ContentEncodingRegistered {
    fn findings(
        ...
        let finding = || -> Option<Violation> {
            ...
            // is which halves of the field each rule reads. This rule reads the
            // response's `Content-Encoding` and the request's `Accept-Encoding`;
            // a stray comma is a defect of either half of either field, so
            // answering it from here would answer it for a response and leave a
            // request silent. `content_encoding_and_type_consistent` walks
            // `Content-Encoding` in both directions and reports it, and
            // `accept_encoding_parameter_valid` owns `Accept-Encoding`'s.
...
mod tests {
    ...
    /// Neither field's stray comma is answered here, and that is an ownership
    /// claim rather than a verdict on the value. This rule reads only the
    /// response's `Content-Encoding` and the request's `Accept-Encoding`, so a
    /// finding made here would leave the other half of each field silent:
    /// `content_encoding_and_type_consistent` walks `Content-Encoding` in both
    /// directions and reports it, and `accept_encoding_parameter_valid` owns
    /// `Accept-Encoding`'s.
    ///
    /// What this rule does take off the value is the `OWS` the list construct
    /// prints around its commas, which is what this test was named for.
    #[test]
    fn the_stray_comma_belongs_to_the_rules_that_read_both_halves() -> anyhow::Result<()> {
        let rule = ContentEncodingRegistered;
        let cfg = make_cfg();
        ...
        assert!(v.is_none(), "the stray comma is not this rule's");

        // The `OWS` the list construct prints around its commas is still
        // trimmed off the coding.
        tx.response.as_mut().unwrap().headers =
