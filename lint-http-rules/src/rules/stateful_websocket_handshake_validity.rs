...
/// request it answers.
///
/// `client_sec_websocket_headers_consistency` measures the request; this rule
...
impl StatefulWebsocketHandshakeValidity {
    ...
    ///
    /// `client_sec_websocket_headers_consistency` reports the same value, and the
    ...
    /// member is `websocket`, which is what `client_sec_websocket_headers_consistency`
    ...
    /// `1#extension`, with `extension-param` productions under it, and nothing in
    /// this catalogue reads it on a request or on a response. An empty list, and a
    /// member deriving from no `extension`, both pass here.
...
mod tests {
    ...
    fn run(tx: &crate::http_transaction::HttpTransaction) -> Option<Violation> {
        StatefulWebsocketHandshakeValidity.check_transaction(
            tx,
    ...
    /// is `client_sec_websocket_headers_consistency`'s.
    #[rstest]
