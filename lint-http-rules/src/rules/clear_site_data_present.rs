...
fn parse_paths_config(
    ...
    let Some(rule_config) = config.get_rule_config(rule_id) else {
        return Err(anyhow::anyhow!(
            r#"rule 'clear_site_data_present' requires configuration to be enabled. Example:
...
paths = ["/logout"]"#
    ...
    let table = rule_config.as_table().ok_or_else(|| {
        anyhow::anyhow!(
            "Configuration must be a table with 'paths' field, e.g., [rules.clear_site_data_present]\npaths = [\"/logout\"]"
        )
    })?;
...
static REGISTRATION: &dyn crate::rules::Rule = &ClearSiteDataPresent;
