...
mod tests {
    ...
    fn make_cfg_with(safe: &[&str]) -> crate::config::Config {
        ...
        cfg.rules.insert(
            "message_early_data_header_safe_method".into(),
            toml::Value::Table({
    ...
    fn check(tx: &crate::http_transaction::HttpTransaction) -> Option<Violation> {
        MessageEarlyDataHeaderSafeMethod.check_transaction(
            tx,
    ...
    fn the_trailer_half_is_the_neighbours_finding() {
        ...
        let neighbour = crate::rules::REGISTERED_RULES
            .iter()
            .find(|r| r.id() == "message_trailer_fields_validity")
    ...
    fn scope_is_both() {
        assert_eq!(
            MessageEarlyDataHeaderSafeMethod.scope(),
            crate::rules::RuleScope::Both
        );
    }
    ...
    fn validate_rules_with_valid_config() -> anyhow::Result<()> {
        MessageEarlyDataHeaderSafeMethod.validate(&make_cfg())
    }
    ...
    fn a_config_without_the_array_is_refused() {
        ...
        let err = MessageEarlyDataHeaderSafeMethod
            .validate(&cfg)
            .expect_err("refused");
    ...
    fn an_empty_array_is_refused() {
        assert!(MessageEarlyDataHeaderSafeMethod
