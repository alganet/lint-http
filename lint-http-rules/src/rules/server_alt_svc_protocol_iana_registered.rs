...
impl Rule for ServerAltSvcProtocolIanaRegistered {
    ...
    fn check_transaction(
        ...
        // `server_alt_svc_header_syntax` reads this field's grammar and reports
...
mod tests {
    ...
    fn check(tx: &crate::http_transaction::HttpTransaction) -> Option<Violation> {
        ServerAltSvcProtocolIanaRegistered.check_transaction(
            tx,
    ...
    /// `server_alt_svc_h3_advertisement_valid` records for its draft-token scan.
    #[test]
    fn an_alpn_protocol_name_is_its_exact_octets() {
        ...
        assert!(ServerAltSvcProtocolIanaRegistered
            .check_transaction(
                &tx,
                &crate::transaction_history::TransactionHistory::empty(),
                &cfg
    ...
    fn what_this_rule_declines_the_grammar_rule_reports(#[case] header: &str) {
        ...
        let syntax = crate::rules::server_alt_svc_header_syntax::ServerAltSvcHeaderSyntax;
        assert!(
            syntax
                .check_transaction(
                    &tx,
    ...
    fn the_allowed_list_is_validated() {
        ...
        for (entry, expected) in [
            ...
            let err = ServerAltSvcProtocolIanaRegistered
    ...
    fn scope_is_server() {
        assert_eq!(
            ServerAltSvcProtocolIanaRegistered.scope(),
            crate::rules::RuleScope::Server
        );
    }
}
