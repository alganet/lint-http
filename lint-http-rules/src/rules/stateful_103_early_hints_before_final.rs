...
static REGISTRATION: &dyn crate::rules::Rule = &Stateful103EarlyHintsBeforeFinal;
...
mod tests {
    ...
    fn run(tx: &crate::http_transaction::HttpTransaction) -> Option<Violation> {
        Stateful103EarlyHintsBeforeFinal.check_transaction(
            tx,
    ...
    fn a_103_after_a_previous_final_response_is_not_a_cross_transaction_finding() {
        ...
        let with_history = Stateful103EarlyHintsBeforeFinal.check_transaction(
            &tx,
    ...
    /// `101` has an owner (`stateful_101_switching_protocols`, which reports one
    ...
    fn published_examples_match_the_rules_verdicts() {
        ...
        for ex in Stateful103EarlyHintsBeforeFinal.examples() {
