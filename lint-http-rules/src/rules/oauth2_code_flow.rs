...
impl Rule for Oauth2CodeFlow {
    fn findings(
        ...
        let finding = || -> Option<Violation> {
            ...
            // but *only once a flow has been seen starting*. `code` is an
            // ordinary parameter name, and a request that merely carries one is
            // not a redirection endpoint: an advertising beacon posting
            // `?code=<base64 url>` is the shape that proved it, drawing an
            // `error` about an OAuth flow that was never in progress.
            //
            // So the same history the correlation below walks is asked one
            // question first: did this client ever send `response_type=code`?
            // That is the request § 4.1.1 defines and the only thing that makes
            // a later `code` a callback rather than a coincidence.
            //
            // **This trades a false positive for a false negative, on purpose.**
            // A flow whose authorization request happened before this proxy
            // started, or under a different client identity, is no longer
            // judged. That is the right way round: the finding is an `error`,
            // and one that fires on traffic which is not OAuth at all costs
            // more than one that stays quiet about a flow it only half saw.
            // cite(RFC 6749 § 4.1.2): "REQUIRED.  The authorization code generated by the authorization server."
            let saw_authorization_request = history.iter().any(|prev| {
                let q = prev.request.uri.split_once('?').map(|x| x.1).unwrap_or("");
                let q = q.split('#').next().unwrap_or(q);
                get_param(&parse_query_string(q), "response_type").is_some_and(|rt| rt == "code")
            });
            if saw_authorization_request && get_param(&params, "code").is_some() {
...
mod tests {
    ...
    fn callback_without_state_flagged() {
        ...
        // A callback is only a callback once a flow has been seen starting.
        let history =
    ...
    fn callback_with_unseen_state_flagged() {
        let rule = Oauth2CodeFlow;
        let tx = make_tx("https://app.example.com/callback?code=abc&state=zzz");
        // A callback is only a callback once a flow has been seen starting.
        let history =
            crate::transaction_history::TransactionHistory::from_transactions(vec![make_tx(
                "https://idp.example.com/authorize?response_type=code&state=xyz",
    ...
    /// The shape that made this rule report an `error` about traffic which was
    /// not OAuth at all: an advertising beacon posting a base64 URL under a
    /// parameter that happens to be spelled `code`, with no authorization
    /// request anywhere behind it.
    #[test]
    fn a_code_parameter_alone_is_not_a_callback() {
        ...
        for uri in [
            "https://developer.mozilla.org/pong/viewed?code=aHR0cHM6Ly9leGFtcGxlLmNvbS9hZA&version=2",
            "https://shop.example.com/checkout?code=SUMMER25",
            "https://api.example.com/lookup?code=en-GB",
        ] {
            assert!(
                crate::test_helpers::run_rule(&Oauth2CodeFlow, &make_tx(uri), &empty, &cfg)
                    .is_none(),
                "reported an OAuth callback for {uri}"
            );
        }
    }
    ...
    fn each_leg_of_the_flow_names_its_entry() {
        let cfg = crate::test_helpers::make_test_config_with_enabled_rules(&["oauth2_code_flow"]);
        let empty = crate::transaction_history::TransactionHistory::empty();
        // The request leg answers for itself; the two callback legs only exist
        // relative to a flow that was seen starting.
        let started =
            crate::transaction_history::TransactionHistory::from_transactions(vec![make_tx(
                "/authorize?response_type=code&state=xyz",
            )]);
        ...
        let callback = crate::test_helpers::run_rule(
            &Oauth2CodeFlow,
            &make_tx("/cb?code=xyz"),
            &started,
            &cfg,
