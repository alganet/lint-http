...
/// and `message_preference_applied_header_valid` is the rule that asks it.
///
...
static REGISTRATION: &dyn crate::rules::Rule = &MessagePreferHeaderValid;
...
mod tests {
    ...
    fn whether_a_preference_was_honored_belongs_to_the_neighbouring_rule() {
        let neighbour = crate::rules::REGISTERED_RULES
            .iter()
            .find(|r| r.id() == "message_preference_applied_header_valid")
            .expect("message_preference_applied_header_valid is registered");
