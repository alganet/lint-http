...
/// `If-Modified-Since` header must be a valid HTTP-date (IMF-fixdate) as defined by RFC 9110 §7.8.1.
...
impl Rule for MessageIfModifiedSinceDateFormat {
        ...
        for hv in tx.request.headers.get_all("if-modified-since").iter() {
            ...
            if !crate::http_date::is_valid_http_date(s.trim()) {
                return Some(Violation {
                    ...
                    severity: config.severity,
                    message:
                        "If-Modified-Since header is not a valid IMF-fixdate (RFC 9110 §7.8.1)"
                            .into(),
                });
            }
        }
    ...
    fn rfc_reference(&self) -> Option<&'static str> {
        Some("[RFC 9110 §7.8.1](https://www.rfc-editor.org/rfc/rfc9110.html#section-7.8.1): If-Modified-Since header")
    }
...
static REGISTRATION: &dyn crate::rules::Rule = &MessageIfModifiedSinceDateFormat;
...
mod tests {
    ...
    fn if_modified_since_cases(
        ...
        let rule = MessageIfModifiedSinceDateFormat;
    ...
    fn id_and_scope_are_expected() {
        let r = MessageIfModifiedSinceDateFormat;
        assert_eq!(r.id(), "message_if_modified_since_date_format");
