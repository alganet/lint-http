...
- [stateful_cookie_lifecycle](rules/stateful_cookie_lifecycle.md) - Track cookies set via `Set-Cookie` and ensure requests include them only while they remain valid; flag expired, deleted, or stale values as well as secure cookies sent over HTTP. (RFC 6265 §5 storage model)
- [stateful_cookie_same_site_enforcement](rules/stateful_cookie_same_site_enforcement.md) - Cookies with `SameSite` attributes must obey cross-site/same-site and navigation restrictions when attached to requests. (RFC 6265bis §5.3.4)
- [stateful_cookie_domain_matching](rules/stateful_cookie_domain_matching.md) - Ensure cookies are only sent when the request host and path satisfy the cookie's domain/path attributes; detects cookies misapplied to wrong origins. (RFC 6265 §5.1.3/§5.1.4)
- [semantic_cache_coherence](rules/semantic_cache_coherence.md) - Responses for a given resource should not regress in their `Date`/`Last-Modified` timestamps; serving an older representation after a newer one indicates cache coherence issues. (RFC 9111 §6)
- [stateful_max_age_directive_validity](rules/stateful_max_age_directive_validity.md) - Cached responses tagged with `Cache-Control: max-age` should only be revalidated after the indicated freshness lifetime and not prematurely (age calculations per RFC 9111 §4.2/§4.3). Detects unnecessary conditional requests and stale unconditional reuse.
- [stateful_immutable_cache_never_stale](rules/stateful_immutable_cache_never_stale.md) - Responses marked `Cache-Control: immutable` should not be revalidated while they remain fresh; detects conditional requests that occur before the freshness lifetime elapses. (RFC 8246)
- [stateful_s_max_age_enforcement](rules/stateful_s_max_age_enforcement.md) - Private clients must ignore `s-maxage`; revalidating after `s-maxage` but before `max-age` indicates the directive was misapplied (RFC 9111 §5.2).
- [stateful_no_store_enforcement](rules/stateful_no_store_enforcement.md) - Responses marked `Cache-Control: no-store` should never be retained; conditional requests using validators from a no-store response indicate prohibited caching. (RFC 9111 §5.2.2.3)
- [stateful_private_cache_visibility](rules/stateful_private_cache_visibility.md) - `Cache-Control: private` responses must not be stored by shared caches; conditional requests from a different client using a private validator reveal caching violations. (RFC 9111 §5.2)
- [stateful_no_cache_revalidation](rules/stateful_no_cache_revalidation.md) - Responses containing `Cache-Control: no-cache` must not be reused without conditional revalidation when validators are available; unconditional requests may indicate improper caching. (RFC 9111 §5.2.2.5)
- [stateful_must_revalidate_enforcement](rules/stateful_must_revalidate_enforcement.md) - Responses bearing the `must-revalidate` directive must not be reused once they are stale without successful revalidation; detects unconditional requests sent after the freshness lifetime has expired. (RFC 9111 §5.2.2.2)
- [stateful_oauth2_code_flow](rules/stateful_oauth2_code_flow.md) - Ensure OAuth2 authorization code flow correlates request and callback state to prevent CSRF/replay. (RFC 6749 §4.1)
...
- [client_request_version_method_validity](rules/client_request_version_method_validity.md) - Ensures request methods are appropriate for the presence of a message body (GET/HEAD/DELETE/TRACE/CONNECT must not carry content) per RFC 9110 §6.3 and related method semantics.
...
- [semantic_patch_partial_update](rules/semantic_patch_partial_update.md) - PATCH requests with a body must declare a patch document media type (type or subtype containing or starting with "patch"). (RFC 5789 §2)
...
- [stateful_101_switching_protocols](rules/stateful_101_switching_protocols.md) - Validate 101 Switching Protocols responses: client must have offered upgrade, response protocol must match, and no HTTP traffic after upgrade on the same connection. (RFC 9110 §15.2.2, §7.8)
...
- [stateful_websocket_handshake_validity](rules/stateful_websocket_handshake_validity.md) - Verify WebSocket upgrade handshake responses are correct and the `Sec-WebSocket-Accept` matches the request `Sec-WebSocket-Key`. (RFC 6455 §4)
- [stateful_websocket_frame_opcode_sequence](rules/stateful_websocket_frame_opcode_sequence.md) - Validate WebSocket frame opcode sequencing: reserved opcodes, control frame payload limits, and data-after-Close detection. (RFC 6455 §5)
- [server_http3_status_code_validity](rules/server_http3_status_code_validity.md) - 101 Switching Protocols must never be sent over HTTP/3; interim 1xx responses must not include body or prohibited headers. (RFC 9114 §4.1, §4.5)
- [stateful_http3_goaway_semantics](rules/stateful_http3_goaway_semantics.md) - Validate HTTP/3 GOAWAY semantics: stream ID must not increase between successive GOAWAYs, and no streams may open beyond the indicated last stream ID. (RFC 9114 §5.2)
- [stateful_http3_max_push_id](rules/stateful_http3_max_push_id.md) - Validate HTTP/3 MAX_PUSH_ID monotonicity: each successive MAX_PUSH_ID frame must be greater than or equal to the previous value (H3_ID_ERROR otherwise). (RFC 9114 §7.2.7)
- [stateful_http3_settings_frame](rules/stateful_http3_settings_frame.md) - Validate HTTP/3 SETTINGS frame semantics: at most one per connection and no reserved HTTP/2-only setting identifiers. (RFC 9114 §7.2.4)
- [server_quic_transport_parameters](rules/server_quic_transport_parameters.md) - Validate that QUIC transport parameters are reasonable for HTTP/3: non-zero stream limits, flow-control windows, and idle timeout. (RFC 9000 §18.2)
...
- [stateful_authentication_failure_loop](rules/stateful_authentication_failure_loop.md) - Detects repeated `401 Unauthorized` challenges for the same protection space (origin), which strongly indicates an authentication failure loop.
- [stateful_digest_auth_nonce_handling](rules/stateful_digest_auth_nonce_handling.md) - Ensures Digest authentication nonces, opaque values and nonce-counts follow proper lifecycle rules (nonce reuse, incrementing `nc`, `opaque` consistency, stale-nonce resets). (RFC 7616 §3.2)
...
- [semantic_options_method_capabilities](rules/semantic_options_method_capabilities.md) - Successful `OPTIONS` responses SHOULD include an `Allow` header advertising supported methods. (RFC 9110 §9.3.7)
...
- [semantic_post_creates_resource](rules/semantic_post_creates_resource.md) - POST responses creating resources should use 201 Created with Location. (RFC 9110 §9.3.3)
...
- [server_alt_svc_h3_advertisement_valid](rules/server_alt_svc_h3_advertisement_valid.md) - Alt-Svc advertising `h3` must use the final protocol ID (not draft versions) and a reasonable `ma` (max-age) value. (RFC 9114 §3.1, RFC 7838)
...
- [message_trailer_fields_validity](rules/message_trailer_fields_validity.md) - Actual trailer fields must not contain prohibited headers (framing, routing, authentication, etc.) and should match the `Trailer` header declaration when present. (RFC 9110 §6.5.1)
...
- [message_http3_host_authority_consistency](rules/message_http3_host_authority_consistency.md) - `:authority` pseudo-header and `Host` header must be consistent when both present in HTTP/3 requests. (RFC 9114 §4.3.1)
...
- [message_http3_pseudo_headers_validity](rules/message_http3_pseudo_headers_validity.md) - HTTP/3 requests must include `:method` and `:path` (or `:authority` for CONNECT); responses must include a valid `:status`. (RFC 9114 §4.3)
...
- [stateful_range_request_and_caching](rules/stateful_range_request_and_caching.md) - Caches that have stored a 206 Partial Content response SHOULD include an appropriate `If-Range` validator when issuing a new Range request; missing or mismatched validators may lead to stale or recombined partial data. (RFC 7233 §3.2; RFC 9111 §3.4, §4.3.1)
- [stateful_cache_validation_chain](rules/stateful_cache_validation_chain.md) - Warns when a conditional request uses a validator value that does not correspond to the most recently observed ETag or Last-Modified for the resource, indicating that the cache's validation chain may be broken. (RFC 9111 §4.3)
- [stateful_vary_header_cache_validity](rules/stateful_vary_header_cache_validity.md) - Conditional requests should preserve the values of all headers listed in a prior response's `Vary` header; differing values indicate the cache key omitted a required dimension. (RFC 9111 §4.1)
