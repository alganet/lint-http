...
- **The exceptions are a violation's severity and whether it reports at all.** A `ViolationDef` carries a `default_severity` and reports unless switched off; `[violations.<id>]` overrides either with `severity` or `enabled`. A rule *option* is policy about the traffic and nobody but the operator can choose it; a severity is a preference, and a defect reporting at all is the reason it was written down — a catalogue with one entry per defect cannot be made to run at all if every entry must be answered first. Nothing else on a def or a rule gets a default by this argument.
- **`enabled = false` on a violation drops that defect's findings and leaves the rest of the rule reporting.** What it cannot do is bring a *different* defect of the same rule into view on a transaction where the switched-off one fired: most rules report their first finding and stop, so the rule never looked further, and that truncation was there before anything was switched off. The drop happens in `RuleContext::reported`, which every dispatch path calls and `no_dispatch_skips_the_enabled_table` proves they do.
