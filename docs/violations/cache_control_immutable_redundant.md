...
immutable sits on a response with no freshness lifetime
...
```toml
[violations.cache_control_immutable_redundant]
# immutable sits on a response with no freshness lifetime
severity = "info"
```
...
- [immutable_requires_freshness](../rules/immutable_requires_freshness.md)
