...
- **The field over HTTP/2 and HTTP/3.** Both versions forbid connection-specific fields outright, which is a different finding from a malformed value and belongs to the rules that own the version. `message_http3_no_connection_header` reports the HTTP/3 half; nothing in this catalogue reports the HTTP/2 half. The connection-option requirement here is gated to HTTP/1.x for the same reason — demanding `Connection: keep-alive` of an HTTP/2 message would be advice to violate RFC 9113 §8.2.2.
