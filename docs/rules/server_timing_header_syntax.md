...
- [RFC 9110 §5.6.1](https://www.rfc-editor.org/rfc/rfc9110.html#section-5.6.1): Lists (#rule ABNF Extension): `#element => [ 1#element ]` is why an empty `Server-Timing` field value is zero metrics and not a defect. Why a comma with nothing between it and the next one *is* one is § 5.6.1.1's, which `list_member_empty` carries
...
- [RFC 9110 §6.5.1](https://www.rfc-editor.org/rfc/rfc9110.html#section-6.5.1): Limitations on Use of Trailers: the sentence that decides whether this field may sit in a trailer section at all. Not asked here — `trailer_fields_valid` owns it — but it is why the second section this rule reads is worth naming: the Server Timing specification puts the field there only in a section marked non-normative
