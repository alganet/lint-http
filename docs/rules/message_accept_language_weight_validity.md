...
**The language range itself is not checked here.** That is `message_language_tag_format_valid`'s subject: it reports an empty range, whitespace inside one, and an over-long subtag, and lets `*` through.
...
- [RFC 4647 §2.1](https://www.rfc-editor.org/rfc/rfc4647.html#section-2.1): Basic Language Range: where `language-range` is defined, by reference from RFC 9110. Its syntax is `message_language_tag_format_valid`'s subject, not this rule's
