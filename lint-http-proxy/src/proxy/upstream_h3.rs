...
//! ALPN `h3`, the same native roots as the H1/H2 path) opens QUIC connections
//! and pools them per origin, sends the request with the hop-by-hop-stripped
//! header section [`super::exchange`] already built, and adapts the h3 response
//! stream back into a [`ResponseBody`] so the shared tee/commit machinery
//! downstream is reused unchanged.
//!
//! *Which* origins get here at all is not decided in this file:
//! [`super::h3_policy`] states that policy, and this file owns the pool — its
//! fourth stage — because that stage decides between live connections.
//!
...
/// Shared HTTP/3 upstream client: one quinn client endpoint, the pool of live
/// connections it opened, and the [`H3Policy`] that decides which origins are
/// reached over H3 at all.
pub(super) struct H3UpstreamClient {
    ...
    /// Connection pool: origin authority → live reusable connection. Stage 4 of
    /// the policy, kept here rather than in [`super::h3_policy`] because it
    /// decides between live quinn connections and their driver tasks.
    ///
    /// A `parking_lot::Mutex`, matching the stores in `lint-http-core` and the
    /// two caches in [`super::h3_policy`]. It was a `std::sync::Mutex` behind
    /// `.lock().unwrap()`, and the unwrap is the part that mattered: a panic
    /// anywhere under this lock poisons it, and every later acquisition panics
    /// too. A single bad request would have taken H3 upstream down for the life
    /// of the process, with each subsequent request re-panicking on a lock
    /// rather than on anything wrong with itself.
    ///
    /// parking_lot does not poison, so the failure mode after such a panic is a
    /// possibly-stale entry instead of a dead subsystem. For a connection pool
    /// that is the right trade: every entry is already evictable and
    /// revalidated on use.
...
impl H3UpstreamClient {
    ...
    pub(super) fn build(
        ...
        Ok(Some(Self {
            endpoint,
            policy: H3Policy::from_config(cfg),
    ...
    /// The selection policy this client routes by — allow/deny, Alt-Svc
    /// discovery and the negative cache. The exchange consults it before
    /// deciding to call [`Self::forward`], and feeds `Alt-Svc` back into it.
    pub(super) fn policy(&self) -> &H3Policy {
        &self.policy
    }
